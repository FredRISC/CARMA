// code credit: UC-Berkeley
// We modified the code so we could measure the performance, cycle counts.
...
void test_ws(bool A_transpose, bool B_transpose) {
 ...
 // asm volatile("vsetvli %0, %1, e64, m4, ta, ma" : "=r"(block_size_p) : "K"(4)); //SEW=64, LMUL=4
...
int main() {
  ...
  for (int A_transpose = 1; A_transpose < 2; A_transpose++) {
    for (int B_transpose = 1; B_transpose < 2; B_transpose++) {
